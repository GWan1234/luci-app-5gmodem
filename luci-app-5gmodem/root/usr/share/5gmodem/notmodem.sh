...
usb_is_modem() {
	...
	# ИНТЕРФЕЙСЫ ИЩЕМ СРЕДИ ДЕТЕЙ УЗЛА, А НЕ РЯДОМ С НИМ.
	#
	# В /sys/bus/usb/devices устройство и его интерфейсы лежат ПЛОСКО, соседями
	# (1-1 и 1-1:2.0), а в /sys/devices интерфейс - РЕБЁНОК устройства
	# (.../1-1/1-1:2.0). Шаблон "$узел":* работал только для первого вида, а
	# сюда узел приходит и от readlink -f (listmodems), то есть вторым.
	# Тогда цикл не находил НИ ОДНОГО интерфейса: ни драйвера, ни класса, и
	# модем известного вендора ещё спасал список CELL_VENDORS, а незнакомого -
	# нет. Живой отчёт 04.09.2026: Dell DW5821e (413c:81d7) работал и раздавал
	# интернет, но «Список модемов» был ПУСТ, активный модем не выбирался, и
	# страница «Сеть» бесконечно перерисовывалась.
	# Шаблон по детям верен для ОБОИХ видов пути: 1-1 в /sys/bus/usb/devices -
	# симлинк на тот же каталог, и его дети те же самые.
