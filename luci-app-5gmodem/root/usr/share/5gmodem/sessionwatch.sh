...
_modem_ip() {
	...
	printf '%s' "$_mi_out" | sed -n 's/.*+CGPADDR: *1,//p' | tr ',' '\n' | tr -d '" ' | grep -E '^[0-9]{1,3}([.][0-9]{1,3}){3}$' | head -1
}
...
check_one() {   # $1 - путь, $2 - интерфейс, $3 - прото, $4 - AT-порт
	...
	# Поднимаем сами: интерфейс с автозапуском либо со снятым автозапуском, но
	# с ошибкой протокола в errors (это работа netifd, а не человека), только
	# если устройство модема на месте, и не чаще кулдауна - тем же _revive, что
	# и остальные случаи.
	_sw_st=$(ifstatus "$_if" 2>/dev/null)
	case "$_sw_st" in
		*'"up": true'*) : ;;
		*)
			_swwhy="лежит, а автозапуск включён"
			case "$_sw_st" in
				*'"autostart": true'*) : ;;
				*)
					# СНЯТЫЙ АВТОЗАПУСК - ЭТО ЕЩЁ НЕ ВОЛЯ ЧЕЛОВЕКА. Сдавшийся
					# протокол зовёт proto_block_restart, и netifd снимает
					# autostart сам: снаружи неотличимо от ручного ifdown, а
					# отличает их errors - у ручного список пуст, здесь в нём
					# лежит причина отказа. Ровно так же различает health.sh.
					_swerr=$(printf '%s' "$_sw_st" \
						| jsonfilter -e '@.errors[0].code' 2>/dev/null)
					[ -n "$_swerr" ] || return 0    # опущен намеренно - не трогаем
					# Явный запрет в конфиге уважаем при любых ошибках.
					[ "$(uci -q get "network.$_if.auto")" = "0" ] && return 0
					[ "$(uci -q get "network.$_if.disabled")" = "1" ] && return 0
					_swwhy="лежит с ошибкой протокола ($_swerr)"
					;;
			esac
