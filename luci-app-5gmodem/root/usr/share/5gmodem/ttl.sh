...
apply)
	...
	mkdir -p /etc/nftables.d
	{
		...
		if [ -n "$DEV" ]; then
			...
			[ -n "$T4I" ] && echo "chain mangle_prerouting { iifname \"$DEV\" counter ip ttl set $T4I; }"
			[ -n "$T4O" ] && echo "chain mangle_postrouting { oifname \"$DEV\" counter ip ttl set $T4O; }"
			[ -n "$T6I" ] && echo "chain mangle_prerouting { iifname \"$DEV\" counter ip6 hoplimit set $T6I; }"
			[ -n "$T6O" ] && echo "chain mangle_postrouting { oifname \"$DEV\" counter ip6 hoplimit set $T6O; }"
		fi
	} > "$NFT"
