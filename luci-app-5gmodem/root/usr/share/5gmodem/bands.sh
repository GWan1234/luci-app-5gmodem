...
_reconnect_iface() {
	...
	# ubus поднимает ровно один интерфейс и глобальной перезагрузки не вызывает -
	# проверено на живом роутере, в т.ч. из состояния down.
	# `ifup` оставлен фолбэком: путь в ubus может отличаться на других сборках.
