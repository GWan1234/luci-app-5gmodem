...
# CACHE-FIRST для status (PERF-DIAGNOSIS Fix 1). Слоты меняются ТОЛЬКО при
...
if [ "$1" = "status" ] && [ -z "$_REFRESH" ]; then
	...
	case "$_SFT" in
		''|*[!0-9]*) : ;;
		*) if [ -s "$_SF" ] && [ "$(( $(cut -d. -f1 /proc/uptime) - _SFT ))" -lt 300 ]; then
			cat "$_SF"
			exit 0
		   fi ;;
	esac
...
if [ "$_VIA" = qmi ]; then
	...
	*)
		...
		if [ -n "$_OUT" ] && [ -n "$_ACT" ]; then
			...
			# qmicli-опрос слотов не делит tty с метриками (свой QMI-канал), пустой
			# ответ здесь = генуинно нет слотов, кэшируем (мгновенные след. открытия).
