...
probe_dev() {
	...
	for _pd_t in $H_TGT; do
		...
		# ПРЕФИКС if! ОБЯЗАТЕЛЕН. Без него curl понимает аргумент и как имя
		# устройства, и как АДРЕС: не сумев привязаться к устройству, он молча
		# привязывается к его IP - а пакет после этого уходит по ЧУЖОМУ
		# маршруту, лишь с нашим адресом в источнике. За NAT это проходит, и
		# мёртвый линк получал вердикт «жив». Живой случай 25.08.2026 (WH3000
		# Pro): кабельный wan получил от DHCP адрес без шлюза, default-маршрута
		# у него нет вовсе - а сторож писал «up», и человек не мог понять,
		# почему приоритет на кабель не переключается. if! - только устройство.
		_pd_tc=$(curl -s -m 3 --interface "if!$_pd_dev" -o /dev/null -w '%{time_connect}' "https://$_pd_t/" 2>/dev/null)
		case "$_pd_tc" in
			''|0|0.000000) ;;
			*)
...
heal() {
	...
	for _h_f in "$HDIR"/*; do
		...
		# АДРЕС В netifd - НЕ ТО ЖЕ САМОЕ, ЧТО АДРЕС НА УСТРОЙСТВЕ.
		#
		# Прото со `proto_set_keep 1` (qmiraw и родня) держит интерфейс «up»
		# после того, как модем пропал с шины и вернулся: в ubus остаётся
		# ПРОШЛЫЙ адрес, а на netdev его давно нет - устройство лежит DOWN и
		# пустое. Вердикт ниже читал только ubus, видел адрес и останавливал
		# лечение навсегда как при лимите оператора. Живой случай 05.09.2026,
		# Quectel EC21 на стенде: модем сам ушёл в USB-сброс, интерфейс так и
		# остался «есть адрес 10.92.155.114, трафика нет», лестница встала, и
		# поднять его было уже некому.
		#
		# Поэтому сверяем адрес с ЯДРОМ. Нет его на l3-устройстве - значит
		# сессии нет, это мёртвый линк, а не лимит: лечим обычным порядком.
		if [ -n "$_h_lip" ]; then
			_h_l3=$(printf '%s' "$_HDUMP" | jsonfilter \
				-e "@.interface[@.interface=\"$_h_if\"].l3_device" 2>/dev/null)
			if [ -n "$_h_l3" ] && ! ip -4 addr show dev "$_h_l3" 2>/dev/null \
				| grep -q "inet $_h_lip"; then
				_h_lip=""
			fi
		fi
		if [ -n "$_h_lip" ]; then
