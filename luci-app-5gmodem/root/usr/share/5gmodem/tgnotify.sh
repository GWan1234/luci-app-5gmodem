...
_tk_one() {   # $1 - usb-путь модема; 1 = дальше идти нельзя (Telegram недоступен)
	...
	# даже при двух модемах (аудит 12.09.2026).
	_tk_tag=""
	case "$(printf '%s' "$_tk_any" | tr '\n' ' ')" in
...
Переключить: /modem <номер>" ;;
			/help*|/start*)
				_co_kw=$("$RES/smscmd.sh" list 2>/dev/null | tr '\n' ' ')
				_tg_reply "Команды:
...
${_co_kw:+
Команды роутера (те же, что по SMS): $_co_kw
Пишите словом, без косой черты; если задан защитный код - перед словом.
}
Модемы:
$(_tg_list)" ;;
			*)
				# КОМАНДЫ ИЗ СПИСКА «Команды по SMS» РАБОТАЮТ И ИЗ ЧАТА.
				#
				# Раньше бот знал только свои /sms, /status и /modem, а
				# «reboot», настроенный на странице команд, молча игнорировал:
				# человек видел /status в ответ и считал, что бот жив, а
				# команда не доходит (жалоба 04.09.2026).
				#
				# Молчим, если слово не команда: чат - обычная переписка, и
				# отвечать «не понял» на каждое сообщение нельзя.
				_co_out=$("$RES/smscmd.sh" chat "$_co_txt" 2>/dev/null)
				case "$_co_out" in
					*'"result":"ok"'*)
					...
					*'"error":"disabled"'*)
						_tg_reply "Команды выключены: включите их на странице «Команды по SMS»." ;;
					*) : ;;
				esac ;;
		esac
	done
