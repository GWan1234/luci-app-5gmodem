...
$_o_grp"
		...
		if [ "$_o_first" = 0 ]; then
			_o_word=$(printf '%s' "$_o_text" | awk '{print $1; exit}')
			_o_rest=$(printf '%s' "$_o_text" | awk '{ $1=""; sub(/^[ \t]+/,""); print; exit }')
	...
	[ "$_o_first" = 1 ] && _log "первая встреча с модемом $MPATH: текущие сообщения помечены обработанными, команды не выполнялись"
	return 0
}
