...
radio_verdict() {   # $1 - АТ-порт
	...
	case "$_rv" in
		...
		0)  echo "CFUN=0 - РАДИО ВЫКЛЮЧЕНО: соединение не поднимется ни одним протоколом"
		    echo "    Часто это СЛЕД оборванного дозвона, а не чей-то выбор: сдавшись,"
		    echo "    протокол гасит радио при разборе сессии. Включает обратно"
		    echo "    перезагрузка модуля - AT+CFUN=1,1 или кнопка «Перезагрузить модем»." ;;
...
at_conn_verdict() {   # $1 - AT-порт
	...
	if [ -n "$_ac_ill" ]; then
		echo "    КАРТА В СОСТОЯНИИ illegal - и это, скорее всего, главная причина."
		echo "    Протокол qmi, увидев illegal, раз в ~8 c режет питание слота и пробует"
		echo "    заново. Карте, которой на инициализацию нужны 1-2 минуты, он не даёт"
		echo "    подняться НИКОГДА - смена протокола (MBIM и прочие) этого не лечит."
		echo "    Снимает illegal только перезагрузка модуля с выдержкой:"
		echo "      sms_tool -d ${1:-<AT-порт>} at \"AT+CFUN=1,1\"   и ЖДАТЬ ~2 минуты"
		echo "    То же самое делает кнопка «Перезагрузить модем» на странице «Модем»,"
		echo "    а сам по себе это чинит сторож интернета (лечение «перезагрузка модуля»)."
		echo "    Если карту переставляли между слотами - сперва проверьте раздел"
		echo "    «Слоты SIM»: активным должен быть тот слот, где она стоит."
	fi
