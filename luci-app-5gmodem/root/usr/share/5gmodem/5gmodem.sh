...
earfcn2band() {   # $1 - EARFCN; печатает номер диапазона LTE
	...
	elif [ "$1" -ge 9870 ]  && [ "$1" -le 9919 ];   then echo 31
...
if [ -e /usr/bin/sms_tool ]; then
	...
	# Тогда добираем короткими группами (по 2-3 команды - столько принимают и
	# слабые модули), сохраняя формат $O. При ПУСТОМ $O порт подвис и опрос
	# вылетел по сторожу - гонять ещё 7 команд по 8 c бессмысленно и вредно (те
	# самые «минуты ожидания»). Пусто -> пропускаем.
	#
	# ПРИЗНАК ПРОВАЛА - НЕТ РЕГИСТРАЦИИ, А НЕ НЕТ СИГНАЛА. По стандарту ошибка
	# любой команды в склейке ОБРЫВАЕТ всё, что стоит за ней. Прежний признак
	# «нет +CSQ» ловил только эхо-модемы: если спотыкалась команда ПОСЛЕ +CSQ,
	# сигнал в ответе был, фолбэк молчал, а оператор, регистрация и IMSI
	# оставались пустыми навсегда. Ровно так выглядел Altair ALT3800 (NTmore
	# NTLM-500, живой отчёт 10.09.2026): «csq 9», остальное прочерками, хотя
	# +COPS?, +CEREG? и +CIMI поодиночке отвечали исправно. Полный ответ без
	# +CREG: и +CEREG: не бывает - их отсутствие и есть оборванная склейка.
	case "$O" in
		"") : ;;
		*"+CREG:"*|*"+CEREG:"*) : ;;
		*)
...
_ATCA_NEG=600
_at_ca_supplement() {
	# ПРОФИЛЬ УЖЕ РАЗОБРАЛ НЕСУЩИЕ - ВТОРОЙ РАЗ НЕ СПРАШИВАЕМ. Флаг ставит только
	# вендорный профиль, который сам сходил за агрегацией той же командой (Telit
	# FN990). Без него единственная несущая оставляла S1BAND пустым, гейт ниже
	# пропускал - и на каждый TTL уходил лишний заход в общий AT-порт.
	[ -n "$_CA_FROM_PROFILE" ] && return 0
