...
if [ -z "$(uci -q show 5gmodem 2>/dev/null | sed -n 's/^5gmodem\.\([^.]*\)=smscmd$/\1/p')" ]; then
	...
	# ЗАЩИТНЫЙ КОД ТОЖЕ ЗАПОЛНЯЕМ - И ИМЕННО СЛОВОМ «secret».
	#
	# Пустое поле рядом со «словом в SMS» ничего не объясняет: их путают, и
	# вопрос «а чем код отличается от команды» возникает у каждого. Заполненное
	# поле отвечает на него без единой строчки документации - в примере команда
	# выглядит как «secret reboot», и роли обоих слов видны сразу.
	#
	# Само слово, разумеется, секретом не является, поэтому подпись поля просит
	# его сменить. Опасности в дефолте нет: раздел команд выключен, а включив
	# его, человек первым делом видит заполненный код.
	#
	# Ставим ТОЛЬКО вместе со своими заготовками. Тому, кто уже завёл команды и
	# шлёт их без кода, внезапный код сломал бы всё, что работало.
	[ -n "$(uci -q get 5gmodem.sms.cmd_secret)" ] || uci -q set 5gmodem.sms.cmd_secret='secret'
