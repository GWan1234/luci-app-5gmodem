...
*[English version](README.md) · [简体中文](README.zh-CN.md) · [日本語](README.ja.md)*
...
- NTmore NTLM-500 (Altair ALT3800, из домашнего роутера Skylink H1) — сигнал, обслуживающая и соседние соты, уровни по антеннам, температура, мощность передатчика, потери на трассе, выбор диапазонов
- Tri Cascade VOS 5G / SG500M2-X (Compal RXM-G1, USB 05c6:9091, ModemManager поверх QMI) — опознаётся по модели, поэтому интерфейс создаётся с `proto=modemmanager`, а не на `uqmi`, которого эта прошивка не обслуживает; ADB-интерфейс освобождается от драйвера последовательных портов. Известное ограничение: source-specific маршруты IPv6 (`default from …`) остаются за netifd и «Приоритетом интернета» не переранжируются

### Заведены по документации производителя
Этих модемов у меня нет, и отчётов по ним пока не было: профили написаны по фирменным справочникам AT-команд и проверены на образцах ответов из них. Логи от владельцев пригодятся.
